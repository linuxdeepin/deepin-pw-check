...
bool is_type_valid(const char *pw, char *character_type,
                   int character_num_required) {

  DEBUG("called,pw is %s,character policy is %s,required is %d", pw,
        character_type, character_num_required);
...
bool is_monotone_character(const char *pw, int monotone_num) {
  ...
  for (int i = 0; i < length - 1; i++) {
    ...
    if (monotone_increase_num >= monotone_num ||
        monotone_decrease_num >= monotone_num) {
      return true;
    }
...
PW_ERROR_TYPE deepin_pw_check(const char *user, const char *pw, int level,
  ...
  DEBUG("read config is\n"
        ...
        "\tdict_path=%s",
        options->enabled, options->min_len, options->max_len,
        options->character_type, options->character_num_required,
        options->check_word, options->monotone_character_num,
        options->consecutive_same_character_num,
        options->first_letter_uppercase, options->dict_path);
  ...
  do {
    ...
    if (PW_NO_ERR !=
        (ret = is_length_valid(pw, options->min_len, options->max_len))) {
      break;
    }
    ...
    if (!is_type_valid(pw, options->character_type,
                       options->character_num_required)) {
    ...
    if (options->monotone_character_num &&
        options->monotone_character_num > 0) {
    ...
    if (options->consecutive_same_character_num &&
      ...
      if (is_consecutive_same_character(
              pw, options->consecutive_same_character_num)) {
...
char *get_pw_validate_policy(int level) {
  ...
  const char *read_type =
      iniparser_getstring(dic, "Password:VALIDATE_POLICY", "");
...
const char *err_to_string(PW_ERROR_TYPE err) {
  ...
  case PW_ERR_LENGTH_SHORT:
    ...
    snprintf(out_buff, BUFF_SIZE,
             gettext("Password must have at least %d characters"), len);
    return out_buff;
  case PW_ERR_LENGTH_LONG:
    len = get_pw_max_length(0);
    snprintf(out_buff, BUFF_SIZE,
             gettext("Password must be no more than %d characters"), len);
    return out_buff;
  case PW_ERR_CHARACTER_INVALID:
    strcpy(tmp_buff, get_pw_validate_policy(0));
    snprintf(out_buff, BUFF_SIZE, gettext("Password can only contain %s"),
             tmp_buff);
    return out_buff;
